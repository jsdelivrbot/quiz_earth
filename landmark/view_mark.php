...
$strpg = "SELECT * FROM province_centroids  a 
          inner join  activities_project b on a.pv_tn = b.province 
          where id_project = $idproject ";
$arr = pg_query($db,$strpg);
$arr = pg_fetch_array($arr);
...
var map = new L.Map('map', {
    center: new L.LatLng(<?php echo $arr[pv_lat]; ?>, <?php echo $arr[pv_lon]; ?>),
    zoom: 9
});
...
var polygonLayer = new L.GeoJSON.AJAX("http://www2.cgistln.nu.ac.th/budgetview/landmark/geojson_project_detail.php?type=polygon&idproject=<?php echo $idproject; ?>",{ 
    onEachFeature: onEachFeature
}).addTo(map);

var lineLayer = new L.GeoJSON.AJAX("http://www2.cgistln.nu.ac.th/budgetview/landmark/geojson_project_detail.php?type=polyline&idproject=<?php echo $idproject; ?>",{ 
    onEachFeature: onEachFeature
}).addTo(map);

var markerLayer = new L.GeoJSON.AJAX("http://www2.cgistln.nu.ac.th/budgetview/landmark/geojson_project_detail.php?type=marker&idproject=<?php echo $idproject; ?>",{ 
    onEachFeature: onEachFeature
}).addTo(map);

function onEachFeature(feature, layer) {   
        var popupContent = '<table class="table"><tbody><tr><td>ชื่อโครงการ  </td><td>'+feature.properties.f3+'</td></tr><tr><td>กิจกรรมหลัก  </td><td>'+feature.properties.f5+'</td></tr><tr><td>กิจกรรมย่อย  </td><td>'+feature.properties.f6+'</td></tr><tr><td>กลุ่มโครงการ </td><td>'+feature.properties.f4+'</td></tr><tr><td>ห่วงโซ่คุณค่า  </td><td>'+feature.properties.f7+'</td></tr><tr><td>ผู้ดูแลโครงการ  </td><td>'+feature.properties.f8+'</td></tr></tbody></table>' ;
